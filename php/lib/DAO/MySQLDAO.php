...
abstract class MySQLDAO implements IDAO
{
    ...
    public static function cleanup($item, $sup = null)
    {
        ...
        if (!isset(static::$supplementary) && $sup == null) {
            return $item;
        }
        if (!$sup) {
            $sup = static::$supplementary;
        }
        foreach ($item as $k => $v) {
